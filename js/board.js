...
function resetDropAreas() {
  const areas = ["to-do-area", "in-progress-area", "awaiting-feedback-area", "done-area"];
...
function renderAssignedUsersOverview(task) {
  const container = document.getElementById("overview-assigned-to-box" + task.id);
  container.innerHTML = "";
  ...
  for (let i = 0; i < task.assignedTo.length; i++) {
    ...
    if (registeredUser) container.innerHTML += getTaskAssignedUsersHTML(registeredUser);
  }
}
...
function getCompletedSubtasks(subtasks) {
  if (subtasks) {
    ...
    subtasks.forEach((subtasks) => {
      if (subtasks.status === "closed") completedSubtasks++;
    });
...
function hideStatusArrows(searchedTasks) {
  let array;
  if (searchedTasks) array = searchedTasks;
  else array = tasks;
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (task.status === "to-do") mobileBox.querySelectorAll("img")[0].classList.add("d-none");
    if (task.status === "done") mobileBox.querySelectorAll("img")[1].classList.add("d-none");
  }
}
