...
function openTask(id) {
  ...
  renderAssignedUsersOverviewOverlay(id, "assigned-to-box", getAssignedUsersHTML);
...
function setOverlayPrioIcon(id) {
  ...
  if (task.prio === "high") container.innerHTML = getOverviewPrioHTML("Urgent", task.prio);

  if (task.prio === "medium") container.innerHTML = getOverviewPrioHTML("Medium", task.prio);

  if (task.prio === "low") container.innerHTML = getOverviewPrioHTML("Low", task.prio);
...
function getOverlayContent(id) {
  for (let i = 0; i < tasks.length; i++) {
    ...
    if (id == task.id) return getOverlayHTML(task);
  }
}
...
function renderAssignedUsersOverviewOverlay(id, boxID, callBack) {
  ...
  task.assignedTo.forEach((user) => {
    ...
    if (registeredUser) userBox.innerHTML += callBack(registeredUser);
  });
}
...
function renderSubtasksInOverlay(container, subtask, taskId, index) {
  if (subtask.status == "open")
    container.innerHTML += getSubtaskHTML(subtask, "updateSubtaskInOverlay", index, taskId);
  else
    container.innerHTML += getSubtaskCheckedHTML(subtask, "updateSubtaskInOverlay", index, taskId);
}
...
async function updateSubtaskInOverlay(subtaskIndex, taskID) {
  ...
  if (checkbox.checked) task.subTasks[subtaskIndex].status = "closed";
  else task.subTasks[subtaskIndex].status = "open";
...
function editTask(taskID) {
  ...
  renderAssignedUsersOverviewOverlay(task.id, "edit-assigned-users-box", getEditUserHTML);
...
function checkAssignedUsers(taskId) {
  if (taskId) {
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      if (assignedTo.indexOf(label.innerText.replace(/(\r\n|\n|\r|\s)/gm, "")) > -1) {
