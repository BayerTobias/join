...
function checkForBypass() {
  ...
  if (currentUrl == "/html/sign_Up.html") return false;
...
function toggleSidebarFocus() {
  ...
  let idClass =
    "sidebar-" + pathName.replace("/html/", "").replace(".html", "").replace("/join", "");

  let idObject =
    "sidebar-object-" + pathName.replace("/html/", "").replace(".html", "").replace("/join", "");

  if (idClass === "sidebar-help") return;
...
function checkIfBoard() {
  ...
  if (htmlPage == "/board.html") return true;
  else return false;
}
...
function updateAssignedUsersDisplay() {
  if (checkIfEditTask()) {
    ...
    container.innerHTML = "";
    const labels = document.getElementById("assigned-to").querySelectorAll("label");

    for (let i = 0; i < labels.length; i++) {
      const label = labels[i];
      const user = getUserIndex(label.innerText.replace(/(\r\n|\n|\r|\s)/gm, ""));
      if (user && label.control.checked) container.innerHTML += getEditUserHTML(user);
    }
  }
}
...
function checkIfEditTask() {
  if (checkIfBoard() && document.getElementById("edit-assigned-users-box")) return true;
  else return false;
}
