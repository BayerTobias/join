...
function renderContacts() {
  ...
  for (let i = 0; i < currentUser.contacts.length; i++) {
    ...
    if (!letters.includes(singleLetter)) letters.push(singleLetter);
  }
  letters.sort();
  contactsList.innerHTML = "";
  renderContactSeparator(letters);
  renderContactItems();
}

/**
 * This function renders the contact list with alphabetical dividers.
 *
 * @param {Array} letters - The array of unique first letters.
 */
function renderContactSeparator(letters) {
  for (let j = 0; j < letters.length; j++) {
    const element = letters[j];
    contactsList.innerHTML += getContactSeparator(element);
  }
}

/**
 * This function renders contact items under their respective dividers.
 */
...
function addNewContact() {
  ...
  currentUser.contacts.push(getContactData(name));
  ...
  if (window.innerWidth > 1100) newContact.scrollIntoView({ behavior: "smooth" }, true);
  showToast();
}

/**
 * Generates contact data object based on provided values.
 *
 * @param {string} name - The name of the contact.
 * @returns {JSON} Contact data object.
 */
...
function closeContact() {
  const overlay = document.getElementById("currentContact");
  overlay.classList.remove("current-contact-animation");
}
